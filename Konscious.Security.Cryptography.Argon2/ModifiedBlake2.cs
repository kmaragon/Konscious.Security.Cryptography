    ...
    internal static class ModifiedBlake2
    {
        ...
        public static void Blake2Prime(Memory<ulong> memory, LittleEndianActiveStream dataStream, int size = -1)
        {
            ...
            if (size <= 64)
            {
                ...
                blake2.Initialize();
                memory.Span.Blit(blake2.ComputeHash(hashStream), 0, 0, size);
            }
            else
            {
                ...
                memory.Span.Blit(chunk, offset, 0, 32); // copy half of the chunk
                offset += 4;
                size -= 32;
                ...
                while (size > 64)
                {
                    ...
                    memory.Span.Blit(chunk, offset, 0, 32); // half again
                ...
                blake2.Initialize();
                memory.Span.Blit(blake2.ComputeHash(chunk), offset, 0, size); // copy the rest
            }
        }
    }
}